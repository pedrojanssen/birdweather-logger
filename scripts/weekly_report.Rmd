...
if (!file.exists(path_master) || file.info(path_master)$size == 0) {
  stop("Master file missing or empty.")
}
...
if (is.na(species_col) | is.na(time_col)) {
  stop("Could not detect species/time columns. Got: ",
...
# new species in this period: first appearance in full dataset falls in [start_date, end_date]
first_all <- df %>%
  group_by(species) %>%
